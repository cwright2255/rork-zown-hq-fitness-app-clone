...
export default function WorkoutsScreen() {
  ...
  const handleFollowAlongScroll = useCallback((event) => {
    const contentOffset = event.nativeEvent.contentOffset.x;
    const index = Math.round(contentOffset / PREVIEW_CARD_INTERVAL);
    setCurrentFollowAlongIndex(index);
  }, []);

  const handleYoutubeScroll = useCallback((event) => {
    const contentOffset = event.nativeEvent.contentOffset.x;
    const index = Math.round(contentOffset / PREVIEW_CARD_INTERVAL);
    setCurrentYoutubeIndex(index);
  }, []);

  const handleRunningProgramScroll = useCallback((event) => {
    const contentOffset = event.nativeEvent.contentOffset.x;
    const index = Math.round(contentOffset / PREVIEW_CARD_INTERVAL);
    setCurrentRunningProgramIndex(index);
  }, []);

  const scrollToFollowAlongIndex = useCallback((index) => {
  ...
  const scrollToYoutubeIndex = useCallback((index) => {
  ...
  const scrollToRunningProgramIndex = useCallback((index) => {
  ...
  const handleRegisterForRace = useCallback((raceId) => {
