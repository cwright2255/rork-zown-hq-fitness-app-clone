...
export default function AdminPanel() {
  ...
  const completeOnboarding = userStore?.completeOnboarding;
  const startOnboarding = userStore?.startOnboarding;
  const logout = userStore?.logout;
  const initializeDefaultUser = userStore?.initializeDefaultUser;
  ...
  const toggleOnboarding = useCallback(() => {
    if (isOnboarded && startOnboarding) {
      startOnboarding();
    } else if (!isOnboarded && completeOnboarding) {
      completeOnboarding();
    }
    ...
    <View style={styles.root} testID="admin-root">
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.section} testID="admin-analytics-section">
          ...
          <Text style={styles.meta}>Weekly workouts (mock): {analytics?.getAnalytics?.('week')?.workoutsCompleted ?? 0}</Text>
        </View>
