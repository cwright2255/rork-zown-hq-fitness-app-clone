...
export default function ActiveRunScreen() {
  ...
  const { startRun, endRun, updateActiveRun } = useRunningStore();
  ...
  const handleEndRun = useCallback(() => {
    ...
    try {
      // Save all captured route coordinates, distance, calories and pace to the activeRun state in store
      updateActiveRun({
        coords: coordinates,
        distance: distance,
        calories: Math.round(distance * 60),
        duration: elapsed,
        pace: distance > 0 ? (elapsed / 60) / distance : 0
      });
      endRun(user?.uid);
  ...
  }, [router, distance, elapsed, coordinates, updateActiveRun, endRun, addExp, user]);
