          ...
          filteredRuns.map((run) => {
            ...
            return (
              <View key={run.id || Math.random().toString()} style={styles.runCard}>
                ...
                {/* Map thumbnail / Path preview */}
                {run.coords && run.coords.length > 1 ? (
                  <View style={styles.mapThumbnail}>
                    <View style={styles.thumbnailPathContainer}>
                      {run.coords.slice(1).map((coord, index) => {
                        const prevCoord = run.coords[index];
                        const latRange = Math.max(...run.coords.map((p) => p.latitude)) - Math.min(...run.coords.map((p) => p.latitude));
                        const lonRange = Math.max(...run.coords.map((p) => p.longitude)) - Math.min(...run.coords.map((p) => p.longitude));

                        const minLat = Math.min(...run.coords.map((p) => p.latitude));
                        const minLon = Math.min(...run.coords.map((p) => p.longitude));
                    ...
                    <View style={styles.thumbnailLabelRow}>
                      ...
                      <Text style={styles.thumbnailText}>GPS Route Captured ({(run.coords.length)} trackpoints)</Text>
                    </View>
                  </View>
                ) : (
