...
const TypedHamburgerMenu = HamburgerMenu as React.ComponentType<HamburgerMenuProps>;
...
export default function RootLayout() {
  ...
  const { cart } = useShopStore() as ShopStoreSlice;
  const { connectSpotify, connectSpotifyImplicit } = useSpotifyStore();
  const [isMenuVisible, setIsMenuVisible] = useState<boolean>(false);
  const [fontsLoaded] = useFonts({});

  const cartItemCount = useMemo(() => cart.reduce((total: number, item: CartItem) => total + item.quantity, 0), [cart]);
