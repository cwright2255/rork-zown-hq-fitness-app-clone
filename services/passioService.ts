...
// Types for Passio API responses
interface PassioTokenResponse {
  access_token: string;
  customer_id: string;
  expires_in: number;
  scope: string;
...
interface PassioAdvisorResponse {
  threadId: string;
  messageId: string;
  content?: string;
  contentToolHints?: string[];
  dataRequest?: any;
  actionResponse?: {
    data: string;
    messageId: string;
...
let foodCache = new Map<string, FoodItem>();
const CACHE_EXPIRY = 5 * 60 * 1000; // 5 minutes
let lastCacheTime = 0;

// Get or refresh access token
const getAccessToken = async (): Promise<{ token: string; customerId: string }> => {
  ...
  try {
    ...
    const data: PassioTokenResponse = await response.json();
...
const convertPassioToFoodItem = (passioFood: any): FoodItem => {
...
const calculateNutritionalScore = (nutrition: any): 'A' | 'B' | 'C' | 'D' | 'E' => {
...
export const testPassioConnection = async (): Promise<{ success: boolean; message: string }> => {
  try {
    const { token, customerId: customerIdValue } = await getAccessToken();
...
const getMockFallback = (): FoodItem[] => [
  {
    id: '1',
    name: 'Chicken Breast',
    servingSize: '100g',
