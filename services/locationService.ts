...
class LocationService {
  ...
  async checkPermissions(): Promise<Location.LocationPermissionResponse | null> {
    try {
      if (Platform.OS === 'web') {
  ...
  private async startWebTracking(): Promise<boolean> {
    if (!navigator.geolocation) {
    ...
    return new Promise((resolve) => {
      const watchId = navigator.geolocation.watchPosition(
        (position) => {
          ...
          this.handleLocationUpdate(location as Location.LocationObject);
  ...
  stopTracking(): void {
    if (Platform.OS === 'web' && (this as any).webWatchId) {
      navigator.geolocation.clearWatch((this as any).webWatchId);
  ...
  addListener(callback: (state: LocationServiceState) => void): () => void {
