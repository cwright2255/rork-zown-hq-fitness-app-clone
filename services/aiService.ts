...
interface LLMResponse { completion: string }
...
export type MacroPresetKey = 'balanced' | 'highProtein' | 'lowCarb' | 'keto' | 'carbLoad' | 'recovery';
...
const withTimeout = async <T>(p: Promise<T>, ms: number, signal?: AbortSignal): Promise<T> => {
  return new Promise<T>((resolve, reject) => {
    const id = setTimeout(() => {
      if (signal && 'aborted' in signal && (signal as AbortSignal).aborted) return;
...
const postLLM = async (messages: LLMMessage[], timeoutMs: number = 30000, retries: number = 1): Promise<LLMResponse> => {
  let lastErr: unknown;
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      const data = await (res as Response).json() as LLMResponse;
...
const normalizeExercise = (e: any, fallbackDifficulty: 'beginner' | 'intermediate' | 'advanced', idx: number): Exercise => {
  ...
  const muscleGroups: string[] = Array.isArray(e?.muscleGroups) ? e.muscleGroups.map((m: any) => String(m)) : [];
  const equipment: string[] = Array.isArray(e?.equipment) ? e.equipment.map((m: any) => String(m)) : [];
  const difficulty = (['beginner','intermediate','advanced'] as const).includes(e?.difficulty) ? e.difficulty : fallbackDifficulty;
...
const getWorkoutImage = (category: string): string => {
...
const getUserDietaryContext = () => {
  try {
    ...
    return { goals: [] as string[], restrictions: [] as string[] };
  }
};

export const chatAI = async (messages: LLMMessage[]): Promise<string> => {
  const data = await postLLM(messages, 30000, 1);
  return data.completion;
};

export const generateWorkoutPlan = async (request: AIWorkoutRequest): Promise<Workout> => {
  try {
    ...
    const difficulty = (['beginner','intermediate','advanced'] as const).includes(parsed?.difficulty) ? parsed.difficulty : request.fitnessLevel;
    ...
    const exercisesSource: any[] = Array.isArray(parsed?.exercises) ? parsed.exercises : [];
    const exercises: Exercise[] = exercisesSource.slice(0, 12).map((e, i) => normalizeExercise(e, difficulty, i));
    const equipment = Array.isArray(parsed?.equipment) ? parsed.equipment.map((x: any) => String(x)) : request.equipment;
...
export const getAIDailyTargets = async (
  params: {
    goals: string[];
  ...
  try {
    let readiness: ReadinessLevel = 'medium';
    let recoveryNote = '';
    try {
      const mood = await wearableService.getMoodDataFromWearables();
      if (mood) {
    ...
    const raw = typeof data.completion === 'string' ? data.completion.trim() : '';
    const parsed = parseJSONSafe<DailyTargets>(raw);
...
export const getAIMacroSplitForPreset = async (
  params: {
    preset: MacroPresetKey;
    calories: number;
  }
): Promise<MacroSplitResult> => {
  try {
    const sys: LLMMessage = {
      role: 'system',
      content:
    ...
    const raw = typeof data.completion === 'string' ? data.completion.trim() : '';
    const parsed = parseJSONSafe<MacroSplitResult>(raw);
    ...
    const ratios: Record<MacroPresetKey, { p: number; c: number; f: number }> = {
...
export const getAIWorkoutPlan = async (
  ...
  try {
    const workout = await generateWorkoutPlan({
      fitnessLevel: (['beginner','intermediate','advanced'] as const).includes(fitnessLevel as any) ? (fitnessLevel as 'beginner'|'intermediate'|'advanced') : 'beginner',
      goals: [goal],
      duration,
      equipment,
    });
...
export const getFitnessMetrics = async (_userId: string): Promise<any> => {
  try {
    return {
