...
export interface WearableDevice {
  id: string;
  name: string;
  type: 'fitbit' | 'apple_watch' | 'garmin' | 'oura' | string;
  connected: boolean;
  lastSync?: string;
}
...
class WearableService {
  ...
  async getMoodDataFromWearables(): Promise<MoodData | null> {
    try {
  ...
  calculateMoodFromMetrics(data: WearableData): number {
    let moodScore = 3;
  ...
  calculateEnergyFromMetrics(data: WearableData): number {
  ...
  calculateStressFromMetrics(data: WearableData): number {
  ...
  calculateSleepFromMetrics(data: WearableData): number {
  ...
  calculateConfidence(data: WearableData): number {
    let dataPoints = 0;
    const totalPoints = 8;
