...
export interface WearableData {
  ...
  distance?: number;
  // Mood-related metrics from wearables
  stressLevel?: number; // 1-5 scale from HRV analysis
  sleepQuality?: number; // 1-5 scale from sleep tracking
  energyLevel?: number; // 1-5 scale from activity/recovery data
  recoveryScore?: number; // 1-100 scale
  hrv?: number; // Heart Rate Variability in ms
  restingHeartRate?: number;
  bodyBattery?: number; // Garmin's body battery or similar
  readinessScore?: number; // Oura's readiness score or similar
}

export interface WearableDevice {
  id: string;
  name: string;
  type: 'fitbit' | 'apple_watch' | 'garmin' | 'oura' | 'samsung';
  connected: boolean;
  lastSync?: string;
}
...
class WearableService {
  ...
  async getAvailableDevices(): Promise<WearableDevice[]> {
    try {
  ...
  async syncData(deviceId: string): Promise<WearableData | null> {
    try {
      if (!deviceId) {
        return null;
      }
  ...
  async getDeviceById(deviceId: string): Promise<WearableDevice | null> {
    try {
      if (!deviceId) {
        return null;
      }
  ...
  async getMoodDataFromWearables(): Promise<{
    mood: number;
    energy: number;
    stress: number;
    sleep: number;
  ...
  private calculateMoodFromMetrics(data: WearableData): number {
  ...
  private calculateEnergyFromMetrics(data: WearableData): number {
  ...
  private calculateStressFromMetrics(data: WearableData): number {
  ...
  private calculateSleepFromMetrics(data: WearableData): number {
  ...
  private calculateConfidence(data: WearableData): number {
